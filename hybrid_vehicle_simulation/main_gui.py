...
class VehicleSimApp:
    def __init__(self, root):
        ...
        # Validation rules: True means >0, False means >=0, tuple is (min, max)
        self.spec_validation_rules = {
            ...
            "road_gradient_percent": None, # No specific numeric check beyond type
        ...
        for widget in self.graph_buttons_frame.winfo_children(): widget.destroy()
    ...
    def _get_and_validate_specs(self) -> dict:
        ...
        try:
            ...
            processed_specs['common_shift_rpm'] = int(raw_specs['common_shift_rpm'])
            processed_specs['shift_time_s'] = float(raw_specs['shift_time_s'])
            processed_specs['road_gradient_percent'] = float(raw_specs['road_gradient_percent'])
            processed_specs['additional_mass_kg'] = float(raw_specs['additional_mass_kg'])
            processed_specs['drivetrain_efficiency_percent'] = float(raw_specs['drivetrain_efficiency_percent'])
            processed_specs['max_total_torque_nm'] = float(raw_specs['max_total_torque_nm'])
            ...
            if not processed_specs['gear_ratios']: raise ValueError("기어비 목록이 비어있거나 모든 항목이 유효하지 않습니다.")
    ...
    def save_specifications(self):
        ...
        if not file_path: messagebox.showinfo("정보", "저장이 취소되었습니다."); return
        try:
    ...
    def load_specifications(self):
        ...
        if not file_path: messagebox.showinfo("정보", "불러오기가 취소되었습니다."); return
        try:
            ...
            for key, value in specs_data.items():
                if key in self.spec_entries:
                    self.spec_entries[key].delete(0, tk.END); self.spec_entries[key].insert(0, value)
            messagebox.showinfo("성공", f"'{file_path}'에서 제원을 불러왔습니다.")
        except FileNotFoundError: messagebox.showerror("오류", "파일을 찾을 수 없습니다.")
        except json.JSONDecodeError: messagebox.showerror("오류", "잘못된 JSON 파일 형식입니다.")
        except Exception as e: messagebox.showerror("오류", f"제원 불러오기 중 오류 발생: {e}")
    ...
    def browse_file(self, entry_key: str):
        ...
        if file_path: self.spec_entries[entry_key].delete(0, tk.END); self.spec_entries[entry_key].insert(0, file_path)
    ...
    def run_simulation_sequence(self):
        ...
        try:
            ...
            transmission = Transmission(
                gear_ratios=specs['gear_ratios'], final_gear_ratio=specs['final_gear_ratio'],
                efficiency=transmission_efficiency )
            self.vehicle_obj_for_plotting = Vehicle(
                base_mass_kg=specs['mass_kg'], additional_mass_kg=specs.get('additional_mass_kg', 0.0),
            ...
            if not isinstance(self.vehicle_obj_for_plotting.engine.torque_curve, pd.Series) or self.vehicle_obj_for_plotting.engine.torque_curve.empty:
                 if specs.get('engine_torque_csv'): messagebox.showwarning("토크 커브 경고", f"엔진 토크 커브 로드 실패 또는 데이터 없음: {specs['engine_torque_csv']}")
            if motor and (not isinstance(self.vehicle_obj_for_plotting.motor.torque_curve, pd.Series) or self.vehicle_obj_for_plotting.motor.torque_curve.empty):
                 if specs.get('motor_torque_csv'): messagebox.showwarning("토크 커브 경고", f"모터 토크 커브 로드 실패 또는 데이터 없음: {specs['motor_torque_csv']}")
            ...
            self.accel_results, self.simulation_results_df = simulate_acceleration(
                ...
                road_gradient_percent=specs['road_gradient_percent'], shift_time_s=specs['shift_time_s'],
            ...
            ref_rpm_grade = specs.get('engine_redline_rpm', 6000) * 0.15 if specs.get('engine_redline_rpm', 0) > 0 else 500.0
            ref_rpm_grade = max(ref_rpm_grade, 800)
            self.max_gradeability_result = calculate_max_gradeability(self.vehicle_obj_for_plotting, reference_rpm_for_torque=ref_rpm_grade )
        ...
        except FileNotFoundError as e: messagebox.showerror("파일 오류", f"파일 찾기 실패: {e.filename}"); self.results_text.config(state=tk.NORMAL); self.results_text.insert(tk.END, f"\n오류: 파일 찾기 실패 - {e.filename}\n"); self.results_text.config(state=tk.DISABLED)
        except ValueError: self.results_text.config(state=tk.NORMAL); self.results_text.insert(tk.END, f"\n입력 값 오류 발생. 메시지를 확인하세요.\n"); self.results_text.config(state=tk.DISABLED)
        except Exception as e: messagebox.showerror("시뮬레이션 오류", f"예상치 못한 오류: {e}"); self.results_text.config(state=tk.NORMAL); self.results_text.insert(tk.END, f"\n시뮬레이션 오류: {e}\n"); self.results_text.config(state=tk.DISABLED)
    ...
    def display_results_in_gui(self):
        ...
        if self.max_gradeability_result is not None:
            if math.isinf(self.max_gradeability_result) and self.max_gradeability_result > 0: results_string += "  발진 가능 최대 구배: 거의 수직 또는 그 이상\n"
            elif self.max_gradeability_result == -999.0: results_string += "  발진 가능 최대 구배: 계산 오류\n"
        ...
        elif abs(current_gradient_for_accel) <= 1e-3: results_string += f"  (평지에서의 최고 속도는 위 '최고 속도 (평지)' 섹션 참조)\n"
        results_string += "\n연비:\n  기본 연비 예측: 제공되지 않음.\n\n-----------------------------------\n"
        self.results_text.insert(tk.END, results_string); self.results_text.config(state=tk.DISABLED)
    ...
    def show_accel_map_plot(self):
        if self.vehicle_obj_for_plotting:
            ...
            if accel_map_data is not None and not accel_map_data.empty: plot_acceleration_map(accel_map_data, title="기어별 최대 가속도")
